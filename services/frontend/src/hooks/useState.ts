  ...
  const setState = (newValue: T | ((prev: T) => T)) => {
    const nextValue =
      typeof newValue === "function"
        ? (newValue as (prev: T) => T)(currentState.value)
        : newValue;
    ...
    if (!Object.is(currentState.value, nextValue)) {
      ...
      if (component) {
        if (
          typeof component.isMountedComponent === "function" &&
          component.isMountedComponent()
        ) {
          hooksManager.scheduleUpdate(component);
        }
      } else {
        console.warn(
          "setState called without component context - component may have been unmounted"
        );
      }
    }
  };
